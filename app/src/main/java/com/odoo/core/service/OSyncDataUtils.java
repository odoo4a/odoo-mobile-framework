...
public class OSyncDataUtils {
    ...
    private List<OdooRecord> checkLocalUpdatedRecords() {
        ...
        try {
            ...
            for (ODataRow row :
                    mModel.select(
                            new String[] {},
                            "_is_dirty = ? and _is_active = ? and id != ?",
            ...
            if (serverIds.size() > 0) {
                ...
                for (Integer server_id : serverIds) {
                    ...
                    ODataRow record =
                            mModel.browse(
                                    new String[] {"_write_date"},
                                    "id = ?",
                                    new String[] {server_id + ""});
                    if (record != null) {
                        Date write_date_obj =
                                ODateUtils.createDateObject(
                                        write_date, ODateUtils.DEFAULT_FORMAT, false);
                        Date _write_date_obj =
                                ODateUtils.createDateObject(
                                        record.getString("_write_date"),
    ...
    private HashMap<String, String> getWriteDate(OModel model, List<Integer> ids) {
        ...
        try {
            ...
            if (model.getColumn("write_date") != null) {
                ...
                Log.i(
                        TAG,
                        "Perm Read hidden fields for write_date and create_date : (Only in Odoo 7.0) for "
                                + ids);
                    ...
                    for (OColumn column : columns) {
                        ...
                        if (column.getRelationType() == null) {
                            ...
                            if (column.isFunctionalColumn() && column.canFunctionalStore()) {
                                ...
                                if (!column.isLocal())
                                    dependValues.put(
                                            column.getName(), record.get(column.getName()));
                                ...
                                Object value =
                                        mModel.getFunctionalMethodValue(column, dependValues);
                                ...
                                switch (column.getRelationType()) {
                                    case ManyToOne:
                                        ...
                                        if (!recordsId.contains(recKey)) {
                                            ...
                                            m2oValue.put(
                                                    m2o_model.getDefaultNameColumn(),
                                                    m2oData.getString("name"));
                                            m2oValue.put("_is_dirty", "false");
                                            m2oRowId =
                                                    m2o_model.insertOrUpdate(
                                                            m2oData.getInt("id"), m2oValue);
                                        } else {
                                    ...
                                    case ManyToMany:
                                        ...
                                        List<Integer> m2mIds =
                                                OListUtils.doubleToIntList(record.getM2M(name));
                                        if (mCreateRelationRecords) {
                                            addUpdateRelationRecord(
                                                    mModel,
                                                    m2mModel.getTableName(),
                                                    column.getType(),
                                                    name,
                                                    null,
                                                    column.getRelationType(),
                                    ...
                                    case OneToMany:
                                        if (mCreateRelationRecords) {
                                            OModel o2mModel =
                                                    mModel.createInstance(column.getType());
                                            List<Integer> o2mIds =
                                                    OListUtils.doubleToIntList(record.getO2M(name));
                                            addUpdateRelationRecord(
                                                    mModel,
                                                    o2mModel.getTableName(),
                                                    column.getType(),
                                                    name,
            ...
            for (String key : updateToServerRecords.keySet()) {
                ...
                for (ODataRow record :
                    ...
                    if (adapter.validateRelationRecords(model, record)) {
                        mOdoo.updateRecord(
                                model.getModelName(),
                                OdooRecordUtils.createRecordValues(model, record),
                                record.getInt("id"));
    ...
    private void addUpdateRelationRecord(
            OModel baseModel,
            String relTable,
            Class<?> model,
            String column,
            String relatedColumn,
            OColumn.RelationType type,
            List<Integer> ids) {
    ...
    public HashMap<String, SyncRelationRecords> getRelationRecordsHashMap() {
        if (mCreateRelationRecords) return relationRecordsHashMap;
        return new HashMap<>();
    }
    ...
    public static class SyncRelationRecords {
        ...
        public SyncRelationRecords(
                OModel baseModel,
                Class<?> relationModel,
                String relationColumn,
                String relatedColumn,
                OColumn.RelationType relationType,
