...
public class ODataRow implements Parcelable {
    ...
    protected ODataRow(Parcel in) {}

    public static final Creator<ODataRow> CREATOR =
            new Creator<ODataRow>() {
                @Override
                public ODataRow createFromParcel(Parcel in) {
                    return new ODataRow(in);
                }

                @Override
                public ODataRow[] newArray(int size) {
                    return new ODataRow[size];
                }
            };
    ...
    public Integer getInt(String key) {
        if (_data.get(key).toString().equals("false")) return 0;
        else return Integer.parseInt(_data.get(key).toString());
    }
    ...
    public String getString(String key) {
        if (_data.containsKey(key) && _data.get(key) != null) return _data.get(key).toString();
        else return "false";
    }
