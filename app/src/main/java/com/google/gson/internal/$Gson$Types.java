...
public final class $Gson$Types {
     ...
     * IntegerSet}, the result for when supertype is {@code Set.class} is {@code Set<Integer>} and
     * the result when the supertype is {@code Collection.class} is {@code Collection<Integer>}.
     */
            ...
            for (int i = 0, length = interfaces.length; i < length; i++) {
                if (interfaces[i] == toResolve) {
                    ...
                    return getGenericSupertype(
                            rawType.getGenericInterfaces()[i], interfaces[i], toResolve);
                }
            }
        }
        ...
        if (!rawType.isInterface()) {
            while (rawType != Object.class) {
                ...
                if (rawSupertype == toResolve) {
                    ...
                    return getGenericSupertype(
                            rawType.getGenericSuperclass(), rawSupertype, toResolve);
                }
