...
public class OForm extends LinearLayout {
    ...
    public OForm(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mContext = context;
        if (attrs != null) {
            TypedArray types = mContext.obtainStyledAttributes(attrs, R.styleable.OForm);
            mModel = types.getString(R.styleable.OForm_modelName);
            mEditable = types.getBoolean(R.styleable.OForm_editableMode, false);
    ...
    private void initForm() {
        ...
        for (String key : mFormFieldControls.keySet()) {
            ...
            if (v instanceof OField) {
                ...
                if (mRecord != null) {
                    if (mRecord.contains(c.getFieldName())) val = mRecord.get(c.getFieldName());
                }
                if (val != null) c.setValue(val);
    ...
    private void findAllFields(ViewGroup view) {
        ...
        for (int i = 0; i < child; i++) {
            ...
            if (v instanceof LinearLayout || v instanceof RelativeLayout) {
                if (v.getVisibility() == View.VISIBLE) findAllFields((ViewGroup) v);
            }
            if (v instanceof OField) {
    ...
    private OValues getValues(boolean validateData) {
        ...
        for (String key : mFormFieldControls.keySet()) {
            ...
            if (val == null || TextUtils.isEmpty(val.toString()) || val.toString().equals("-1")) {
                val = false;
            }
    ...
    private void setOnDomainFilterCallBack(final OColumn column, final OField baseField) {
        ...
        for (String key : domainFilter.getFilterColumns()) {
            ...
            if (filterDomain.operator_value == null
                ...
                columnField.setOnFilterDomainCallBack(
                        new IOnDomainFilterCallbacks() {
                            @Override
    ...
    private void setOnChangeForControl(final OColumn column, OField field) {
        field.setOnChangeCallbackListener(
                new IOnChangeCallback() {
                                ...
                                new Handler()
                                        .postDelayed(
                                                new Runnable() {
                                                    ...
                                                    @Override
                                                    public void run() {
                                                        OnChangeBackground bgProcess =
                                                                new OnChangeBackground(column);
